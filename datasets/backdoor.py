...
from utils.label_consistent import labelConsistentAttack
from utils.ssba import SSBA_attack_replace_version
from utils.config import BDBlocker_path
from utils.utils import sigTriggerAttack
...
class BackdooredDataset(torch.utils.data.Dataset):
    def __init__(self, data_root_path, dataset_name='cifar10',
        ...
        super(BackdooredDataset, self).__init__()
        ...
        if trigger_pattern in ['badnet_sq', 'badnet_grid', 'cl', 'backdoor101','sig']:
            pass
        else:
            if trigger_pattern == 'smooth':
                ...
                if trigger_pattern == "blend":
                    trans = transforms.Compose([
                            transforms.ToPILImage(),
                            transforms.Resize((32,32)),  # (32, 32)
                            transforms.ToTensor()])
                    trigger = trans(imageio.imread(os.path.join(BDBlocker_path, 'triggers', '%s_%s.jpeg' % (trigger_pattern, dataset_name.lower())))).cpu().numpy().transpose(1, 2, 0)
                
                if trigger_pattern == "lc":
                    
                    add_trigger = labelConsistentAttack(reduced_amplitude= 1)
                    add_trigger_func = add_trigger.poison_from_indices
                    train_bd_transform = transforms.Compose([
                        (transforms.Resize((32,32)), False),
                        (np.array, False),
                        (SSBA_attack_replace_version(
                            replace_images=np.load(os.path.join(BDBlocker_path, 'triggers', 'cifar10_train.npy'))  # '../data/cifar10_SSBA/train.npy'
                        ), True),
                        (add_trigger_func, False),
                        (npClipAndToUint8,False),
                    ])
                    test_bd_transform = transforms.Compose([
                        (transforms.Resize((32,32)), False),
                        (np.array, False),
                        # (SSBA_attack_replace_version(
                        #     replace_images=np.load(os.path.join(BDBlocker_path, 'triggers', 'cifar10_test.npy'))  # '../data/cifar10_SSBA/test.npy'
                        # ), True),
                        (add_trigger_func, False),
                        (npClipAndToUint8,False),
                    ])
            ...
            trigger = trigger.astype(np.float64)
        self.images = self.images.astype(
            np.float64)  # before adding triggers, change dtype to float to prevent overflow of uint8.
        ...
        if split == 'test':
            ...
            elif trigger_pattern in ['sig']:
                    sig_transform = sigTriggerAttack()
                    for i in range(len(self.images)):
                        self.images[i] = sig_transform.sigTrigger(self.images[i])
            elif trigger_pattern in ["lc"]:
                add_trigger = labelConsistentAttack(reduced_amplitude= 1)
                add_trigger_func = add_trigger.poison_from_indices
                train_bd_transform = transforms.Compose([
                        (transforms.Resize((32,32)), False),
            ...
            # tets_type= type(self.images)
            # save_path = "./test/cifar"
            # txt_path = "./test/cifar"
            # save_name = "trigger.png"
            # test_trigger = 0.2*trigger
            # pil_image = Image.fromarray(test_trigger.astype(np.uint8))
            # if not (
            ...
            # array1 = test_trigger[:, :, 0]
            # array2 = test_trigger[:, :, 1]
            # array3 = test_trigger[:, :, 2]

            # np.savetxt(txt_path+f'/data_1.txt', array1, fmt='%d', delimiter='\t')
            # np.savetxt(txt_path+f'/data_2.txt', array2, fmt='%d', delimiter='\t')
            # np.savetxt(txt_path+f'/data_3.txt', array3, fmt='%d', delimiter='\t')

            
            # file_path = f"{save_path}/{save_name}"
            ...
            if trigger_pattern == 'badnet_sq':
                ...
                train_bd_transform = transforms.Compose([
                        ...
                        (np.array, False),
                        # (SSBA_attack_replace_version(
                        #     replace_images=np.load(os.path.join(BDBlocker_path, 'triggers', 'cifar10_test.npy'))  # '../data/cifar10_SSBA/train.npy'
                        # ), True),
                        (add_trigger_func, False),
                        (npClipAndToUint8,False),
                    ])
