...
class poisonedCLSDataContainer:
    ...
    def setitem(self, key, value, relative_loc_to_save_folder_name=None):
        ...
        if self.save_folder_path is None:
            ...
            save_txt_path = save_subfolder_path +f"/txt"
            if not (
                ...
                and 
                os.path.exists(save_txt_path)
                 and
                os.path.isdir(save_txt_path)
            ):
            ...
            trigger = np.asarray(img)
            array1 = trigger[:, :, 0]
            array2 = trigger[:, :, 1]
            array3 = trigger[:, :, 2]

            np.savetxt(save_subfolder_path+f'/txt/data_1.txt', array1, fmt='%d', delimiter='\t')
            np.savetxt(save_subfolder_path+f'/txt/data_2.txt', array2, fmt='%d', delimiter='\t')
            np.savetxt(save_subfolder_path+f'/txt/data_3.txt', array3, fmt='%d', delimiter='\t')
    ...
    def __getitem__(self, key):
        if self.save_folder_path is None:
            ...
            #file_path = self.data_dict[key]["path"].replace("record/", "record/WRN16-2/")
    ...
    def prepro_backdoor(self):
        for selected_index in tqdm(self.original_index_array, desc="prepro_backdoor"):
            if self.poison_indicator[selected_index] == 1:
                ...
                #???img未做norm
                # img = Image.fromarray(np.zeros((32,32,3)).astype(np.uint8)) # ???
