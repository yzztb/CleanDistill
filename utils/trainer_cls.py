    ...
    def test(self, test_data, device):
        ...
        with torch.no_grad():
            for batch_idx, (x, target, *additional_info) in enumerate(test_data):
                ...
                if str(model)[:str(model).find("(")] in ["ResNets"]:
                    pred, *activations = model(x)
                else:
                    pred = model(x)
...
def given_dataloader_test(
    ...
    with torch.no_grad():
        for batch_idx, (x, target, *additional_info) in enumerate(test_dataloader):
            ...
            for i in range(num_classes):
                ...
                correct_per_class[i] += ((predicted == i) & (target == i)).sum().item()
    ...
    metrics['test_acc_class'] = per_class_acc.cpu().numpy()
    if verbose == 0:
...
class ModelTrainerCLS_v2():
    ...
    def one_forward_backward(self, x, labels, device, verbose=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.amp):
            if str(self.model)[:str(self.model).find("(")] in ["ResNets"]:
                log_probs, *activations = self.model(x)
            else:
                log_probs = self.model(x)
            loss = self.criterion(log_probs, labels.long())
        self.scaler.scale(loss).backward()
